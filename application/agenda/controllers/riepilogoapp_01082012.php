...
function set_filtro(){
	...
	function gest_app( ) {
        ...
        function _getdata_appuntamenti($ruolo,$num_rows_fil,$off,$lim) {
            ...
            $data['ruolo']=$ruolo;
            $data['appuntamenti']=$this->cm->getdata_appuntamenti($this->filter,$off,$lim);
        ...
        function notifica($id,$rec) {
         ...
         $subject = "Schedulato appuntamento alle ore-".$ora.":".$min."-del:".substr($rec['time'],0,8);
         $message = "Richiedente:".$rec['richiedente']."\nArgomento:".$rec['descrizione'];
         //Casella del sistema
