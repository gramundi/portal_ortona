...
$('.day_num').click(function() {
			...
			titolare="<?php echo $titolare ?>";
                        ...
                        datatodayint=parseInt(gg)+parseInt(mm)+parseInt(yy);

                        datesceltaint=parseInt(day_num)+parseInt(month)+parseInt(year);

                        if ((datesceltaint-datatodayint)<0){
...
function valida(){
 ...
 if ((tipoop=='add')||((tipoop=='mod')&&((ora != ora_app_inm)||(min != min_app_inm))))
    $.ajax({
                    type: "POST",
                    url: "<?php echo site_url()."/agenda/valida_ora"; ?>",
                    data: "par="+date+'-'+id_titolare+'-'+ora+'-'+min,
                    dataType:"jsondata",
                    async: false,
...
<table>
        <tr><th>Data Evento</th>
            <td>
            <?php $opt = array('name'=>'data','size'=> '8'); $ro='id="dataevt" readonly="readonly"';
