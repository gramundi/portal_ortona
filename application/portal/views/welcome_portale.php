...
function post_url(link){

//alert(link);
news='<object class="news" data='+link+' width="800" height="480"></object>';
reset_area_cont();
$('#content').append(news);
$('#meteo').hide();

//$('#content').html('<p>ciao</p>');

/* content='';
$.ajax({
            type: "POST",
            async: false,
            url: "<?php echo site_url()."/welcome_portal/get_news"; ?>",
            data:"par="+link,
...
function initializefeed() {
      ...
      var feed = new google.feeds.Feed("http://www.ansa.it/web/notizie/rubriche/economia/economia_rss.xml");
      feed.load(function(result) {
        if (!result.error) {
          url_base="<?php echo site_url()."/welcome_portal/get_news"; ?>";
          for (var i = 0; i < result.feed.entries.length; i++) {
            var entry = result.feed.entries[i];

            $('#feed').append('<div class="textItem"><a href="javascript:void(0);" onclick="post_url('+"'"+entry.link+"'"+');" >'+entry.title+'</a></div><br>');
            //$('#feed').append('<div class="textItem"><a href="'+entry.link+'" >'+entry.title+'</a></div><br>');
...
<div class="des">
<form method="post" action='<?php echo site_url()."/welcome_portal/previsioni/".$ruolo_p  ?>'  >
    <h5>Servizio Meteo
